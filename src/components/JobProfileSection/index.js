...
class JobProfileSection extends Component {
  ...
  getJobDetails = async () => {
    ...
    // Use the data from localStorage if available, otherwise use the static data
    ...
    try {
      ...
      if (employmentType.length > 0) {
        filteredJobs = filteredJobs.filter(job =>
          employmentType.includes(job.employment_type.toUpperCase())
        )
      }
      ...
      if (salaryRange !== '') {
        ...
        filteredJobs = filteredJobs.filter(job => {
          const salary = parseInt(job.package_per_annum, 10)
  ...
  renderJobsList = () => {
    ...
    return (
      <ul className="jobs-list">
        {jobsList.map(job => {
          // For employer view, add applicants information
          if (userRole === 'employer') {
            return (
              <li key={job.id} className="job-list-items employer-view">
